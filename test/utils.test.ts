...
test("test get region from swr image url", async() => {
  ...
  const swradd2 = "docker pull swr.cn-north-4.myhuaweicloud.com/hcloudcli/springbootdemo:v1.1";
  expect(utils.getRegionFromEndpoint(swradd2,1,".")).toEqual("cn-north-4");

  const swradd3 = "docker pull swr.cn-north-2.myhuaweicloud.com/hcloudcli/jdk19:v1.0.0.1";
...
test("test version compare mimetype", async() => {
  ...
  expect(utils.compareVersion(v1,v2)).toEqual(1);

  const v3:string = "20.10.14"
  const v4:string = "20.10.14"
  expect(utils.compareVersion(v3,v4)).toEqual(0);

  const v5:string = "20.10.14"
  const v6:string = "20.20"
  expect(utils.compareVersion(v5,v6)).toEqual(-1);

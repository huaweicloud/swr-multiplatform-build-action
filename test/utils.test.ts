...
describe('check inputs', () => {
  ...
  const testCase = [
      { description: 'imagetag is null', inputs: {imagetag: '', platforms: '', uselatestbuildx: true, push: true, file: ''}, result: false },
      { description: 'region from endpoint does not support', inputs: {imagetag: 'swr.cn-north-a', platforms: '', uselatestbuildx: true, push: true, file: ''}, result: false },
      { description: 'region from endpoint does not support', inputs: {imagetag: 'swr.cn-north-4', platforms: 'linux/amd64', uselatestbuildx: true, push: true, file: ''}, result: false },
  ];
...
describe('check whether platform support', () => {
  const testCase = [
      ...
      { description: '输入单个不支持的平台', platforms: 'alinux/amd64', result: false },
      { description: '输入多个支持的平台', platforms: 'linux/amd64,darwin/amd64', result: true },
      { description: '输入多个存在不支持的平台', platforms: 'linux/amd64,ssdarwin/amd64', result: false },
  ];
  testCase.forEach((item) => {
...
describe('test dockerfile exist', () => {
  ...
  test('test dockerfile exist when file does not exist', () => {
    ...
    expect(utils.checkDockerfileExist('')).toBe(false);
    expect(core.setFailed).toHaveBeenNthCalledWith(1, 'Get information about the given file failed.');
  });
});
...
describe('test get region from endpoint', () => {
  test('test get region from endpoint when region is cn-north-4', () => {
    expect(utils.getRegionFromEndpoint('swr.cn-north-4.xxx/hcloudcli/jdk19:v1.0.0.1',1,'.')).toEqual("cn-north-4");
  });
});
...
describe('test get OSArch buildx', () => {
  const testCase = [
    { description: 'test get OSArch buildx when platform is linux and  OSArch is x64', osPlatform: 'linux', osArch: 'x64', result: 'amd64' },
    { description: 'test get OSArch buildx when platform is darwin and  OSArch is x64', osPlatform: 'darwin', osArch: 'x64', result: 'amd64' },
    { description: 'test get OSArch buildx when platform is linux and  OSArch is arm64', osPlatform: 'linux', osArch: 'arm64', result: 'arm64' },
    { description: 'test get OSArch buildx when platform is darwin and  OSArch is arm64', osPlatform: 'darwin', osArch: 'arm64', result: 'arm64' },
    { description: 'test get OSArch buildx when platform is win32 and  OSArch is x64', osPlatform: 'win32', osArch: 'x64', result: 'amd64.exe' },
    { description: 'test get OSArch buildx when platform is win32 and  OSArch is arm64', osPlatform: 'win32', osArch: 'arm64', result: 'arm64.exe' },
    { description: 'test get OSArch buildx when OSArch is ppc64', osPlatform: 'darwin', osArch: 'ppc64', result: 'ppc64le' },
];
  testCase.forEach((item) => {
...
describe('test get OS platform buildx', () => {
  const testCase = [
    { description: 'test get OS platform buildx when platform is win32 ', osPlatform: 'win32', result: 'windows' },
    { description: 'test get OS platform buildx when platform is linux ', osPlatform: 'linux', result: 'linux' },
    { description: 'test get OS platform buildx when platform is darwin ', osPlatform: 'darwin', result: 'darwin' },
];
  testCase.forEach((item) => {
...
describe('test compare version', () => {
  const testCase = [
    { description: 'version1 > version2', version1: '20.10.14', version2: '20.10.11', result: 1 },
    { description: 'version1 = version2', version1: '20.10.14', version2: '20.10.14', result: 0 },
    { description: 'version1 = version2', version1: '20.10.11', version2: '20.10.14', result: -1},
    { description: 'version1 < version2 and version1 lacks revison', version1: '20.10', version2: '20.10.14', result: -1},
    { description: 'version1 > version2 and version2 lacks revison', version1: '20.10.14', version2: '20.10', result: 1},
];
  testCase.forEach((item) => {
